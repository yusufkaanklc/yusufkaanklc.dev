...
export default function SkillsPage() {
  ...
  const load = () => fetch("/api/admin/skills").then((r) => r.json()).then(setItems);
  useEffect(() => { load(); }, []);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h2 className="text-xl font-bold text-accent">Skill Categories</h2>
        <button
      ...
      <DataTable
        columns={[
          ...
          { key: "skills", label: "Skills", render: (s) => s.skills.join(", ") },
        ]}
        data={items}
